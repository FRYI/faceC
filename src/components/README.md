...
####3.chart包:存放各种图表相关的组件,条形图柱形图折线图等等 具体用法参考首page
####4.countDown包:一个倒计时组件，用法参考homepage,简单描述,该组件有3个属性,
target(时间/毫秒数)必填，
...
####9.layouts+page包：系统page面布局相关组件，比如登陆进去之后page面顶部显示什么，底部显示什么，菜单点击触发多个tab的布局等等 一般情况不需要修改
...
####13.setting包：该包下封装了首page风格切换等功能如下图
...
2.该page面css定义方式,只定义一个顶层class,其余样式都定义在其下,这样只要顶层class不和别的page面冲突,整个page面的样式都是唯一生效的
````
FooterToolBar.vue:fixed定位的底部，通过是否定义内部控件的属性slot="extra"决定是左浮动或是右浮动
HeaderNotice.vue:首pagenotice(如下图)
